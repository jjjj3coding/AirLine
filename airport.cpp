...
char* read_flight()
{
	...
	while (1) {
		scanf("%s", number);
		if (strlen(number) != 8)  printf("您输入的航班号格式有误，请重新输入：\n");
...
int search_flight(char* flightNum)
{
	...
	while (l <= r) {
		...
		else if (strcmp(flightNum, flightArr[mid].flightNum) < 0) r = mid - 1;
		else l = mid + 1;
	}
	return -1;
}
...
{
	int l = 0, r = passengerArr[0].amount;
	int mid;
	while (r - l > 1) {
		mid = (l + r) >> 1;
	...
	if (flightNum != NULL) {
		while (strcmp(id, passengerArr[mid].id) == 0 &&
			strcmp(flightNum, passengerArr[mid].flightNum) != 0)
			mid++;
	}
	if (strcmp(passengerArr[mid].id, id) == 0) return mid;
	return -1;
}
//利用二分，根据账号，返回在文件中的下标 
...
{
	double time1;
	int year1 = time_change(time, 0, 3);
	int month1 = time_change(time, 5, 6);
	int day1 = time_change(time, 8, 9);
	int hour1 = time_change(time, 12, 13);
	int min1 = time_change(time, 15, 16);
	int sum1 = hour1 * 60 + min1;
	time1 = year1 * 1e8 + month1 * 1e6 + day1 * 1e4 + sum1;
	return time1;
}
//添加航班 
void flight_input()
{
	...
	while (1) {
		...
		if (flightArr[flinum].up_time[4] != '/' || flightArr[flinum].up_time[7] != '/' ||
			...
			int days[13] = { 0,31,28,31,30,31,30,31,31,31,30,31 };
	...
	while (1)
	{
		...
		if (flightArr[flinum].off_time[4] != '/' || flightArr[flinum].off_time[7] != '/' ||
			flightArr[flinum].off_time[10] != '/' || flightArr[flinum].off_time[13] != ':')
			printf("您输入的时间格式有误，请重新输入：\n");
		else {
			time2 = computing_time(flightArr[flinum].up_time);
			int days[13] = { 0,31,28,31,30,31,30,31,31,31,30,31 };
			if ((year % 400 == 0 || (year % 4 == 0 && year % 100 != 0))) days[2] = 29;
	...
	while (flightArr[flinum].discount < 0 || flightArr[flinum].discount>100) {
		printf("折扣输入有误，请重新输入\n");
		scanf("%lf", &flightArr[flinum].discount);
	}
...
void flight_change()
{
	...
	while (1)
	{
		...
		else if (choice == 4)
		{
			...
			if (time3 < time2) printf("您的起飞时间落后于降落时间，请修改\n");
			else exit(0);
		}
		Sleep(1000);
		system("cls");
...
int main()
{
	...
	case 8: flight_input(); break;
	case 9: flight_change(); break;
	case 10: del_flight(); break;
	case 11: user_input(); break;
	}
	//endi();
	return 0;
}
